...
st.title(f"{PAGE_ICON} Kognitiver Beleg-Parser (v3.2 - Stable Session)")
...
def ask_gemini_vision_cached(file_bytes: bytes, mime_type: str, skr_mode: str, api_key_trigger: str) -> tuple:
    """
    동일한 파일 바이트와 설정에 대해 Gemini API 호출 결과를 캐싱하여 
    화면 갱신(Rerun) 시 중복 호출 및 데이터 유실을 방지합니다.
    """
    ...
    try:
        ...
        return beleg_nr, d_str, ven, tot, cur, kat, m_type, response.text, True # True: 실제로 API를 호출함 의미
    except Exception:
...
if uploaded_files:
    ...
    # 유저가 수정 중인 데이터가 세션에 없다면 스캔을 최초 1회만 진행
    if st.session_state.get("edited_receipts") is None:
        ...
        with st.spinner("🔮 Verarbeite Dokumente via Kognitiver AI-Engine..."):
            for idx, uploaded_file in enumerate(uploaded_files):
                ...
                # 💡 캐싱된 함수 호출로 구조 변환 (수정 동작에 의해 재호출 시 무조건 0초 처리됨)
                ...
                # 처음 캐시 없이 실제 API 통신을 할 때만 속도 제한 프리티어 딜레이 작동
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # INTERAKTIVE DATEV-ERFASSUNGSMASKE (DATA EDITOR - FULLY STABLE)
    # ══════════════════════════════════════════════════════════════════════════════
    # 유저가 타이핑, 복사, 붙여넣기를 마음껏 해도 st.session_state.edited_receipts가 굳건히 버텨줍니다.
    st.data_editor(
